...
gbif_sizes = CSV.read(joinpath(dir, "gbif_sizes.csv"), DataFrame)[:, 2:end]
###################### ZERO DIAGONAL ##################################
######################################################################
...
# Read the CSV file
diets = CSV.File(joinpath(dir, "TetraEU_generic_diet.csv")) |> DataFrame
diets = hcat(diets.sourceTaxonName, diets.targetGenericItemName)

Amph = CSV.read(joinpath(dir, "DB_Amphibians_IP.txt"), delim='\t', DataFrame)
Bird = CSV.read(joinpath(dir, "DB_Birds_IP.txt"), delim='\t', DataFrame)
Mamm = CSV.read(joinpath(dir, "DB_Mammals_IP.txt"), delim='\t', DataFrame)
Rept = CSV.read(joinpath(dir, "DB_Reptiles_IP.txt"), delim='\t', DataFrame)
...
####################################################################
####################################################################
species_df = CSV.File(joinpath(dir, "Species_spain_df.csv")) |> DataFrame
...
utmraster = Raster(joinpath(dir, "updated_utmraster.tif"))
utmraster_DA = DimArray(utmraster)
utmraster_da = map(x -> isnothing(x) || isnan(x) ? false : true, utmraster_DA)

DA = deserialize(joinpath(dir, "DA.jls"))
DA_herps = deserialize(joinpath(dir, "DA_herps.jls"))
DA_birmmals = deserialize(joinpath(dir, "DA_birmmals.jls"))
...
DA_richness = deserialize(joinpath(dir, "DA_richness.jls"))::DimArray{Float64,2}
...
######################### NPP ####################################
npp_absolute = CSV.File(joinpath(dir, "npp_absolute_df.csv")) |> DataFrame
...
npp_DA = deserialize(joinpath(dir, "npp_DA.jls"))
# npp_DA = npp_DA./10000
...
iberian_interact_df = deserialize(joinpath(dir, "iberian_interact_df.jls"))
...
bio6_DA = deserialize(joinpath(dir, "bio6.jls"))
bio12_DA = deserialize(joinpath(dir, "bio12.jls"))
...
######## niches_df  ##############
species_niches = CSV.File(joinpath(dir, "iberian_species_niches_withbinned_TH.csv"), decimal = ',') |> DataFrame
order_indices = indexin(spain_names, species_niches[:, :Species])
species_niches = species_niches[order_indices, :]

lax_species_niches = CSV.File(joinpath(dir, "iberian_species_niches_withLaxNiche.csv"), decimal = ',') |> DataFrame
order_indices = indexin(spain_names, lax_species_niches[:, :Species])
lax_species_niches = lax_species_niches[order_indices, :]

strict_species_niches = CSV.File(joinpath(dir, "iberian_species_niches_withVeryStrictNiche.csv"), decimal = ',') |> DataFrame
